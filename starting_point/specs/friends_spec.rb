...
class TestFriends < MiniTest::Test
  ...
  def test_favorite_tv_show_return
...
def test_add_new_friend
  num_friends = @person4[:friends].length
  result = add_new_friend(@person4,"Scrappy-Doo")

  assert_equal(num_friends + 1,@person4[:friends].length )
end
...
def test_loan_money
  starting_money_p4 = 20
  starting_money_p5 = 100

  loan_money(@person4,@person5,20) #output final bank amount

  assert_equal(true,starting_money_p4!=@person4[:monies])
  assert_equal(true,starting_money_p5!=@person5[:monies])

end

  # 8. Find the set of everyone's favourite food joined together
  # (hint: concatenate the favourites/snack arrays together)
def test_favorite_snack
  result = favourite_snack(@people)
...
def test_no_friends
  no_friends_array = no_friends(@people)

  assert_equal([person5],no_friends_array)
